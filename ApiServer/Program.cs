...
app.MapPost("/api/invoke", async (HttpRequest request) =>
{
    ...
    await ApiServer.Logger.LogAsync("api.log", $"Incoming invoke: url={url} headerName={headerName} apiKey={maskedKey} input={input}");
    ...
    try
    {
        ...
        await ApiServer.Logger.LogAsync("api.log", $"Forwarded to {url} returned {(int)resp.StatusCode} {resp.ReasonPhrase}: {bodySnippet}");
    ...
    {
        await ApiServer.Logger.LogAsync("api.log", $"Exception: {ex}");
